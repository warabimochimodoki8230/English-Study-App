...
const addFile = path.resolve(new URL('../ADD_WORDS.tsv', import.meta.url).pathname);
if (fs.existsSync(addFile)) {
  ...
  for (const line of lines) {
    ...
    const prev = merged.get(key);
    if (!prev) merged.set(key,{q:entry,files:['ADD_WORDS.tsv'],levels:new Set([level])});
    else {
      prev.files.push('ADD_WORDS.tsv'); prev.levels.add(level);
