...
for (const file of files(root)) {
  ...
  data.forEach((q, i) => {
    ...
    if (q.type === 'vocab' || q.word) {
      ...
      if (seenWords.has(key)) infos.push(`${file}#${i}: repeated headword/pos ${word} (also ${seenWords.get(key)})`);
...
console.log(`INFO: ${infos.length} intentional duplicate headword/pos occurrences retained for review; records are not deleted.`);
if (errors.length) {
