...
async function fetchData() {
  ...
  if (state.section === 'reading') {
    state.questions = raw.flatMap(p => {
      ...
      return p.questions.map(q => ({ ...q, type: 'reading', level: q.level || p.level, passageId: p.id, title: p.title, passage: p.passage, passageTags: p.tags || [] }));
...
function showReadingQuestion(q, opts = {}) {
  ...
  area.innerHTML = `${passageHtml}<div class="q-meta"><span class="tag">${escapeHtml(q.level || 'reading')}</span><span>${opts.study ? '読解トレーニング' : `${state.index + 1} / ${state.questions.length}`}</span><span class="tag">長文</span></div><div class="question">${escapeHtml(view.question)}<small>${escapeHtml(view.prompt)}</small></div><div class="answers" id="answers"></div><div id="explain"></div><div class="next-row"><button id="nextBtn" class="primary-btn" style="display:none">次の問題</button></div>`;
  const box=$('answers');
  ...
  $('nextBtn').onclick=()=>{ state.index++; if(state.index>=state.questions.length) state.index=0; showQuestion(state.questions[state.index], opts); };
}
