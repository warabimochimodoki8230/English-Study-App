...
const vocabModes = {
  ...
  context: { label: '文脈・空所補充', minLevel: 0 },
  synonym: { label: '類義語', minLevel: 0 },
  family: { label: '語族・派生語', minLevel: 0 },
...
const APP_VERSION = '20260822-v26';
const scheduler = {
...
function renderLearningAnalytics() {
  ...
  panel.innerHTML=`<div class="analytics-reset-wrap"><button id="analyticsResetMobile" class="analytics-reset-btn" type="button">進捗をリセット</button></div>
    <div class="analytics-head"><div><div class="analytics-title">学習状況・分析</div><div class="analytics-sub">ジャンル別の実績と、実際のミス・復習履歴から算出した要注意候補を分けて表示します。</div></div></div>
  ...
  const analyticsReset = $('analyticsResetMobile');
  if (analyticsReset) analyticsReset.onclick = resetProgressHandler;
}
...
async function loadAllVocabForAnalytics() {
  ...
  state.allVocab = dedupeRuntimeQuestions(settled.flat().map(normalizeQuestion).filter(Boolean));
  state.vocabLookup = new Map(state.allVocab.filter(isVocab).map(q => [normalizeText(q.word), q]));
}
...
async function fetchData() {
  ...
  if (state.section === 'reading') {
    ...
    state.questions = dedupeRuntimeQuestions(raw.map(normalizeQuestion).filter(Boolean));
  }
  filterQuestions();
}
...
function normalizeQuestion(q) {
  ...
  if (q.type === 'vocab') {
    const word = String(q.word || q.answer || '').trim();
    const meaning = String(q.meaning || q.answerJa || q.ja || '').trim();
    return { ...q, type: 'vocab', word, meaning, pos: normalizePos(q.pos || inferPos(word)) };
  }
  if (q.type === 'idiom') {
...
function buildQuizPool() {
  ensureDifficultySet();
  let pool = state.questions.filter(isVocab).filter(q => state.quiz.difficulty.includes(q.level || state.levels[0])).filter(q => !['article','det'].includes(normalizePos(q.pos)));
...
function inferPos(word) {
  ...
  if (/^(a|an|the)$/.test(raw)) return 'article';
  if (/(tion|sion|ment|ness|ity|ance|ence|ship|hood|dom|ism|ist|acy|age|al)$/.test(raw)) return 'n';
  if (/ly$/.test(raw)) return 'adv';
  if (/(ous|ive|able|ible|ful|less|ic|ical|ish|ary|ant|ent|al)$/.test(raw)) return 'adj';
  if (/(ize|ise|ify|en)$/.test(raw)) return 'v';
  return 'n';
}

function dedupeRuntimeQuestions(rows) {
  const out = []; const seen = new Set();
  for (const q of rows || []) {
    if (!q) continue;
    let sig;
    if (q.type === 'vocab') sig = `vocab::${normalizeText(q.word)}::${normalizePos(q.pos)}::${normalizeText(q.meaning)}`;
    else if (q.type === 'idiom') sig = `idiom::${normalizeText(q.word || q.phrase)}::${normalizeText(q.meaning || q.ja || q.answer)}`;
    else sig = `${q.type || 'q'}::${q.id || normalizeText(q.question || q.prompt)}`;
...
function canonicalAnswer(choices, answer = '') {
  ...
  if (explicitKey) {
    const exactChoice = (Array.isArray(choices) ? choices : []).map(choiceText).find(text => normalizeText(text) === explicitKey);
    if (exactChoice) return exactChoice;
  }
  const flagged = (Array.isArray(choices) ? choices : []).find(c => c && typeof c === 'object' && c.correct === true);
...
function lookupVocab(word, pool = []) {
  ...
  if (!n) return null;
  const direct = (pool || []).find(x => isVocab(x) && normalizeText(x.word) === n);
  return direct || state.vocabLookup?.get(n) || state.allVocab.find(x => isVocab(x) && normalizeText(x.word) === n) || null;
}
function familyMeta(word, pool) {
  const hit = lookupVocab(word, pool);
  return hit ? { word, pos: normalizePos(hit.pos), level: hit.level, record: hit } : { word, pos: inferPos(word) };
}
function levelOrderAdjacent(level) {
  const order = ['basic','standard','developed','advanced','hard'];
  const idx = order.indexOf(level);
  return idx < 0 ? null : order[Math.min(order.length - 1, Math.max(0, idx + 1))];
}
function uniqueVocabPool(primary = []) {
  const out = []; const seen = new Set();
  for (const q of [...(primary || []), ...(state.allVocab || [])]) {
...
function rankCandidate(c, q, targetPos, preferredLevels) {
  const samePos = normalizePos(c.pos) === normalizePos(targetPos) ? 0 : 100;
  const r = levelRank[c.level] ?? 2;
  const distances = preferredLevels.filter(Boolean).map(x => Math.abs(r - (levelRank[x] ?? 2)));
  const levelScore = distances.length ? Math.min(...distances) * 4 : 8;
  const answerLike = relatedValues(q,'family').some(f => normalizeText(f) === normalizeText(c.word)) ? 8 : 0;
  return samePos + levelScore + answerLike;
}
function fillChoicesSmart(correct, candidates, q, targetPos = '', preferredLevels = [], exclude = []) {
  ...
  const out = [];
  const records = (candidates || []).filter(Boolean).sort((a,b) => rankCandidate(a,q,targetPos,preferredLevels) - rankCandidate(b,q,targetPos,preferredLevels));
  ...
  if (out.length < 3) {
    for (const c of uniqueVocabPool(state.allVocab).sort((a,b)=>rankCandidate(a,q,targetPos,preferredLevels))) {
      const text = choiceText(c); const n = normalizeText(text);
      if (!text || seen.has(n)) continue;
...
function fillChoices(correct, candidates, minPool = [], answerRelated = []) {
  ...
  const out = [];
  for (const c of [...candidates, ...minPool].filter(Boolean).sort(() => Math.random() - 0.5)) {
    const text = choiceText(c), n = normalizeText(text);
    if (!text || seen.has(n)) continue;
    seen.add(n); out.push(text); if (out.length >= 3) break;
  }
  return [answer, ...out].slice(0,4);
}
function modeCandidateRecords(q, pool, targetPos = '') {
  const cur = normalizeText(q.word);
  const all = uniqueVocabPool(pool).filter(x => normalizeText(x.word) !== cur);
  const same = targetPos ? all.filter(x => normalizePos(x.pos) === normalizePos(targetPos)) : all;
  const preferred = same.filter(x => x.level === q.level || x.level === levelOrderAdjacent(q.level));
  return [...preferred, ...same.filter(x => !preferred.includes(x)), ...all.filter(x => !same.includes(x))];
}
function modeCandidateValues(q, pool, mode) {
  const records = modeCandidateRecords(q, pool, normalizePos(q.pos));
  if (mode === 'en-ja') return records.map(x => x.meaning).filter(Boolean);
  if (mode === 'ja-en' || mode === 'context') return records.map(x => x.word).filter(Boolean);
  if (mode === 'synonym') return records.flatMap(x => Array.isArray(x.synonyms) ? x.synonyms : []);
  if (mode === 'family') return records.flatMap(x => String(x.family || '').split(';').map(s=>s.trim()).filter(Boolean));
  return [];
}
function modeCanGenerateFourChoices(q, mode, pool) {
  ...
  const answer = mode === 'en-ja' ? q.meaning : mode === 'ja-en' || mode === 'context' ? q.word : mode === 'synonym' ? q.synonyms?.[0] : String(q.family || '').split(';').map(s=>s.trim()).filter(Boolean)[0];
  if (!answer) return false;
  const related = mode === 'synonym' ? q.synonyms.slice(1) : mode === 'family' ? String(q.family||'').split(';').map(s=>s.trim()).filter(Boolean).slice(1) : [];
  let count = 0; const seen = new Set([normalizeText(answer), ...related.map(normalizeText)]);
  const values = modeCandidateValues(q,pool,mode);
  for (const v of values) { const n=normalizeText(v); if(!n||seen.has(n)) continue; seen.add(n); if(++count>=3) return true; }
  return true; // smart fallback can draw from global same-POS pool
}
function pickVocabMode(q) {
  ...
  if (selected === 'random') {
    ...
    const choices = ready.length ? ready : ['en-ja'];
...
function vocabQuestion(q) {
  ...
  const records = modeCandidateRecords(q,pool,targetPos);
  const preferredLevels = [q.level, levelOrderAdjacent(q.level)].filter(Boolean);
  if (mode === 'en-ja') return {mode,question:q.word,prompt:'最も適切な日本語の意味を選べ。',answer:q.meaning,pos:targetPos,choices:fillChoicesSmart(q.meaning,records.map(x=>({...x,label:x.meaning})),q,targetPos,preferredLevels,[q.word])};
  if (mode === 'ja-en') return {mode,question:q.meaning,prompt:'この日本語の意味に合う英単語を選べ。',answer:q.word,pos:targetPos,choices:fillChoicesSmart(q.word,records.map(x=>({...x,label:x.word})),q,targetPos,preferredLevels,[q.meaning])};
  if (mode === 'context') {
    const example = q.exampleSentence || '';
    const rx = new RegExp(String(q.word).replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i');
    const sentence = rx.test(example) ? example.replace(rx,'_____') : example;
    return {mode,question:sentence,prompt:'文脈に最も適する語を選べ。',answer:q.word,pos:targetPos,choices:fillChoicesSmart(q.word,records.map(x=>({...x,label:x.word})),q,targetPos,preferredLevels,[q.meaning]),extra:q.exampleJa||''};
  }
  if (mode === 'synonym') {
    ...
    const target = familyMeta(answer,pool);
    const synRecords = modeCandidateRecords(q,pool,target.pos);
    const distractors = synRecords.flatMap(x=>Array.isArray(x.synonyms)?x.synonyms:[]).map(x=>({word:x,label:x,pos:target.pos,level:q.level}));
    return {mode,question:q.word,prompt:'最も近い意味の語を選べ。',answer,choices:fillChoicesSmart(answer,distractors,q,target.pos,preferredLevels,synonyms.slice(1)),pos:targetPos};
  }
  ...
  const answer = family[Math.floor(Math.random()*family.length)];
  const target = familyMeta(answer,pool);
  const familyRecords = modeCandidateRecords(q,pool,target.pos);
  const familyValues = familyRecords.flatMap(x=>String(x.family||'').split(';').map(s=>s.trim()).filter(Boolean)).map(x=>({word:x,label:x,pos:familyMeta(x,pool).pos,level:q.level}));
  return {mode,question:`「${q.word}」の語族・派生語として適切なものを選べ。`,prompt:'同じ語族から適切な派生語を選べ。',answer,choices:fillChoicesSmart(answer,familyValues.concat(familyRecords.filter(x=>normalizeText(x.word)!==normalizeText(answer))),q,target.pos,preferredLevels,family),hidePos:true};
}
...
function showQuestion(q = state.questions[state.index], opts = {}) {
  ...
  $('emptyState').style.display = 'none';
  const posHtml = (q.type === 'vocab' && view.pos && !view.hidePos) ? `<small class="vocab-pos">${escapeHtml(posLabel(view.pos))}</small>` : '';
  area.innerHTML = `<div class="q-meta"><span class="tag">${escapeHtml(q.level || state.levels[0])}</span><span>${opts.customQuiz ? `${state.quiz.answered + 1} / ${state.quiz.remaining}` : opts.study ? '復習スケジュール' : `${state.index + 1} / ${state.questions.length}`}</span><span class="tag">${escapeHtml(vocabModes[view.mode]?.label || view.mode)}</span></div><div class="question">${escapeHtml(view.question)}${posHtml}<small>${escapeHtml(view.prompt || '')}</small></div><div class="answers" id="answers"></div><div id="explain"></div><div class="next-row"><button id="nextBtn" class="primary-btn" type="button" disabled>次の問題</button></div>`;
  const box = $('answers');
...
function answer(q, isCorrect, btn, view, opts = {}) {
  ...
  if (nextBtn) { nextBtn.disabled = false; nextBtn.classList.add('is-ready'); }
  saveProgress();
}
...
const resetProgressHandler = () => {
  if (!confirm('学習進捗をすべて削除します。\nこの操作は元に戻せません。本当に消去しますか？')) return;
  state.score = {};
  saveProgress();
  filterQuestions();
  if (state.view === 'analytics') renderAll();
};
$('resetProgress').onclick = resetProgressHandler;
...
(async () => {
  ...
  catch (e) { updateInfo(`データの読み込みに失敗しました：${e?.message || e}`); $('emptyState').querySelector('h2').textContent='データを読み込めませんでした'; $('emptyState').querySelector('p').textContent='public/data を含めて配置されているか確認してください。'; console.error(e); }
})();
