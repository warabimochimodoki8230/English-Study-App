...
const progressKey = 'eng-god-progress-v2';
const APP_VERSION = '20260820-vocab-jaen-v4';
...
function isVocab(q) {
  return Boolean(q && (q.type === 'vocab' || String(q.word || '').trim()));
}
...
function eligibleVocab(q, mode) {
  ...
  if (mode === 'en-ja') return Boolean(word && meaning);
  if (mode === 'ja-en') return Boolean(word && meaning);
  if (mode === 'context') return Boolean(String(q.word || '').trim() && String(q.exampleSentence || '').trim());
  ...
  return Boolean(String(q.word || '').trim());
}
...
function filterQuestions() {
  ...
  if (state.section === 'vocab') {
    const eligible = vocabFilteredQuestions().length;
    $('datasetInfo').textContent = `${sections.vocab.title} / ${state.level} / ${vocabModes[state.vocabMode]?.label || 'ランダム'}：${eligible}問`;
  }
}
...
function startQuiz() {
  const source = state.section === 'vocab' ? vocabFilteredQuestions() : state.filtered;
  if (!source.length && state.section === 'vocab' && state.vocabMode === 'ja-en') {
    const fallback = state.filtered.filter(isVocab).filter(q => String(q.word || '').trim() && String(q.meaning || '').trim());
    if (fallback.length) {
      state.questions = [...fallback].sort(() => Math.random() - 0.5);
      state.index = 0;
  ...
  if (!source.length) {
    $('emptyState').style.display = 'block';
    $('emptyState').querySelector('h2').textContent = state.section === 'reading' ? '長文データは準備中です' : 'この形式で使える問題がありません';
    $('emptyState').querySelector('p').textContent = state.section === 'vocab' ? 'このレベルには選択中の形式に必要なデータがありません。英→日を選ぶか、別レベルを試してください。' : '該当する問題がありません。';
    return;
  }
  state.questions = [...source].sort(() => Math.random() - 0.5);
  state.index = 0;
  $('emptyState').style.display = 'none';
  showQuestion();
}
...
function vocabQuestion(q) {
  ...
  const pool = state.questions.filter(isVocab).filter((x) => String(x.word || '').trim().toLowerCase() !== currentWord);
  ...
  if (mode === 'ja-en') {
    return {
      mode,
      question: String(q.meaning || q.answerJa || q.ja || '').trim(),
      prompt: 'この日本語の意味に合う英単語を選べ。',
      answer: String(q.word || q.answer || '').trim(),
      choices: fillChoices(String(q.word || q.answer || '').trim(), pool.map((x) => String(x.word || x.answer || '').trim()))
    };
  }
  ...
  if (mode === 'synonym') {
    ...
    if (!synonyms.length) return { ...vocabQuestion({ ...q, _forceMode: 'en-ja' }), mode: 'en-ja' };
  ...
  if (!family.length) return { ...vocabQuestion({ ...q, _forceMode: 'en-ja' }), mode: 'en-ja' };
...
$('vocabModeSelect').onchange = (e) => { state.vocabMode = e.target.value; resetQuestionArea(); renderAll(); filterQuestions(); };
