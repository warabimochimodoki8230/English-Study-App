...
const APP_VERSION = '20260820-vocab-jaen-v2';
...
function eligibleVocab(q, mode) {
  if (!q || q.type !== 'vocab') return false;
  if (mode === 'en-ja') return Boolean(String(q.word || '').trim() && String(q.meaning || '').trim());
  if (mode === 'ja-en') return Boolean(String(q.word || '').trim() && String(q.meaning || '').trim());
...
function vocabFilteredQuestions() {
  ...
  let qs = state.filtered.filter(x => x.type === 'vocab');
...
function vocabQuestion(q) {
  const pool = state.questions.filter((x) => x.type === 'vocab' && x.word && x.word.toLowerCase() !== q.word.toLowerCase());
  ...
  if (mode === 'ja-en') {
    return {
      mode,
      question: String(q.meaning || '').trim(),
      prompt: 'この意味に合う英単語を選べ。',
      answer: String(q.word || '').trim(),
      choices: fillChoices(String(q.word || '').trim(), pool.map((x) => String(x.word || '').trim()))
    };
  }
