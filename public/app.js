...
function eligibleVocab(q, mode) {
  ...
  const pos = normalizePos(q.pos);
  if (!word || ['a','an','the'].includes(word.toLowerCase()) || pos === 'article') return false;
...
function normalizePos(pos) {
  ...
  const map = {
    '名':'n','名詞':'n','noun':'n','n':'n',
    '動':'v','動詞':'v','verb':'v','v':'v',
    '形':'adj','形容詞':'adj','adjective':'adj','adj':'adj',
    '副':'adv','副詞':'adv','adverb':'adv','adv':'adv',
    '前':'prep','前置詞':'prep','preposition':'prep','prep':'prep',
    '接':'conj','接続詞':'conj','conjunction':'conj','conj':'conj',
    '代':'pron','代名詞':'pron','pronoun':'pron','pron':'pron',
    '限定詞':'det','determiner':'det','det':'det',
    '間':'interj','間投詞':'interj','interjection':'interj','interj':'interj',
    '冠':'article','冠詞':'article','article':'article',
    'ord':'ord','序数':'ord'
  };
  return map[p] || p || 'n';
}
...
function lookupVocab(word, pool = []) {
  ...
  return (pool || []).find(x => isVocab(x) && normalizeText(x.word) === n) || state.vocabLookup?.get(n) || (state.allVocab || []).find(x => isVocab(x) && normalizeText(x.word) === n) || null;
}
...
function modeCandidateValues(q, pool, mode) {
  ...
  const recs = modeCandidateRecords(q, pool, targetPos);
...
function modeCanGenerateFourChoices(q, mode, pool) {
  ...
  const related = mode === 'synonym' ? q.synonyms.slice(1) : mode === 'family' ? relatedValues(q,'family').slice(1) : [];
  let values = modeCandidateValues(q,pool,mode);
  if (mode === 'synonym') values = values.concat(modeCandidateRecords(q,pool,familyMeta(answer,pool).pos||normalizePos(q.pos)).map(x => x.word));
  if (mode === 'family') {
    const target = familyMeta(answer,pool);
    const recs = modeCandidateRecords(q,pool,target.pos||normalizePos(q.pos));
...
function vocabQuestion(q) {
  ...
  if (mode === 'synonym') {
    ...
    const records = modeCandidateRecords(q,pool,synPos);
    const distractors = records.flatMap(x => Array.isArray(x.synonyms) ? x.synonyms : []);
    return {mode,question:q.word,prompt:'最も近い意味の語を選べ。',answer,choices:fillChoicesSmart(answer,distractors.map(x=>({word:x,label:x,pos:synPos,level:q.level})),q,synPos,levels,synonyms.slice(1))};
  }
  const family = relatedValues(q,'family').filter(x => normalizeText(x) !== normalizeText(q.word));
  if (!family.length) return vocabQuestion({...q,_forceMode:'en-ja'});
  const answer = family[Math.floor(Math.random()*family.length)];
  const target = familyMeta(answer,pool);
  const familyRecords = modeCandidateRecords(q,pool,target.pos||targetPos);
  const candidates = [];
  for (const x of familyRecords) {
    for (const f of relatedValues(x,'family')) {
      const meta = familyMeta(f,pool);
      if (normalizePos(meta.pos) === normalizePos(target.pos)) candidates.push({word:f,label:f,pos:meta.pos,level:meta.level||q.level});
    }
  }
  return {mode,question:`「${q.word}」の語族・派生語として適切なものを選べ。`,prompt:'同じ語族から適切な派生語を選べ。',answer,hidePos:true,choices:fillChoicesSmart(answer,candidates.concat(familyRecords),q,target.pos||targetPos,levels,family)};
}
...
function resetAllProgress(){ if(confirm('学習進捗をすべて削除します。よろしいですか？')) { state.score={}; saveProgress(); state.index=0; state.quiz.active=false; state.quiz.answered=0; state.quiz.correct=0; state.quiz.remaining=0; state.quiz.pool=[]; resetQuestionArea({clearData:false}); filterQuestions(); renderAll(); } }
const resetProgressButton=$('resetProgress'); if(resetProgressButton) resetProgressButton.onclick=resetAllProgress;
const resetProgressMobileButton=$('resetProgressMobile'); if(resetProgressMobileButton) resetProgressMobileButton.onclick=resetAllProgress;
