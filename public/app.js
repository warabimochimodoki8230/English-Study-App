...
const APP_VERSION = '20260821-v22-2';
const scheduler = {
...
function lookupVocab(word, pool=[]) {
  const n=normalizeText(word); if (!n) return null;
...
function uniqueVocabPool(primary=[]) {
  ...
  for (const q of [...(primary||[]),...(state.allVocab||[])]) {
    if (!isVocab(q)) continue; const k=normalizeText(q.word); if (!k||seen.has(k)) continue; seen.add(k); out.push(q);
  }
  return out;
}
function fillChoicesSmart(correct, candidates, q, targetPos = '', preferredLevels = [], exclude = []) {
  ...
  const out = [];
  const uniq = candidates.filter(Boolean).filter(x => !seen.has(normalizeText(choiceText(x)))).sort((a,b)=>candidateRank(a,q,targetPos,preferredLevels)-candidateRank(b,q,targetPos,preferredLevels));
  for (const c of uniq) { const text=choiceText(c); const n=normalizeText(text); if(!text||seen.has(n)) continue; seen.add(n); out.push(text); if(out.length>=3) break; }
  return [answer,...out].slice(0,4);
}
function fillChoices(correct, candidates, minPool = [], answerRelated = []) {
  const answer = String(correct || '').trim(); const seen=new Set([normalizeText(answer),...answerRelated.map(normalizeText)]); const out=[];
  for (const c of [...candidates,...minPool].filter(Boolean).sort(()=>Math.random()-0.5)) { const text=choiceText(c), n=normalizeText(text); if(!text||seen.has(n)) continue; seen.add(n); out.push(text); if(out.length>=3) break; }
  return [answer,...out].slice(0,4);
}
function modeCandidateRecords(q,pool,mode,targetPos='') {
  const cur=normalizeText(q.word); const all=uniqueVocabPool(pool).filter(x=>normalizeText(x.word)!==cur);
  const samePos=targetPos?all.filter(x=>normalizePos(x.pos)===targetPos):all;
  const levels=[q.level,levelOrderAdjacent(q.level)].filter(Boolean);
  const preferred=samePos.filter(x=>levels.includes(x.level));
  return [...preferred,...samePos,...all.filter(x=>!samePos.includes(x))];
}
function modeCandidateValues(q,pool,mode) {
  const targetPos=normalizePos(q.pos); const recs=modeCandidateRecords(q,pool,mode,targetPos);
  ...
  if(mode==='synonym') return recs.flatMap(x=>Array.isArray(x.synonyms)?x.synonyms:[]);
...
function modeCanGenerateFourChoices(q, mode, pool) {
  ...
  let values = modeCandidateValues(q,pool,mode);
  if (mode === 'synonym') {
    const target = familyMeta(answer,pool); const recs = modeCandidateRecords(q,pool,'synonym',target.pos||normalizePos(q.pos));
    values = values.concat(recs.map(x=>x.word));
  }
  if (mode === 'family') {
    const target = familyMeta(answer,pool); const recs = modeCandidateRecords(q,pool,'family',target.pos||normalizePos(q.pos));
    values = values.concat(recs.flatMap(x=>String(x.family||'').split(';').map(s=>s.trim()).filter(Boolean)), recs.map(x=>x.word));
  }
  const seen=new Set([normalizeText(answer),...related.map(normalizeText)]); let count=0;
...
function vocabQuestion(q) {
  const pool=state.quiz.active?state.quiz.pool:state.questions.filter(isVocab);
  const mode=pickVocabMode(q); const targetPos=normalizePos(q.pos); const recs=modeCandidateRecords(q,pool,mode,targetPos); const levels=[q.level,levelOrderAdjacent(q.level)].filter(Boolean);
  ...
  if(mode==='context'){
    const example=q.exampleSentence; const rx=new RegExp(q.word.replace(/[.*+?^${}()|[\\]\\]/g,'\\$&'),'i');
  ...
  if(mode==='synonym'){
    const synonyms=Array.isArray(q.synonyms)?q.synonyms.filter(Boolean):[]; if(!synonyms.length)return vocabQuestion({...q,_forceMode:'en-ja'});
    const answer=synonyms[0]; const target=familyMeta(answer,pool); const synPos=target.pos||targetPos; const records=modeCandidateRecords(q,pool,mode,synPos);
    const distractors=records.flatMap(x=>Array.isArray(x.synonyms)?x.synonyms:[]);
    return {mode,question:q.word,prompt:'最も近い意味の語を選べ。',answer,pos:targetPos,choices:fillChoicesSmart(answer,distractors.map(x=>({word:x,label:x,pos:synPos,level:q.level})).concat(records.map(x=>({...x,label:(Array.isArray(x.synonyms)?x.synonyms[0]:x.word)}))),q,synPos,levels,synonyms.slice(1))};
  }
  const family=String(q.family||'').split(';').map(s=>s.trim()).filter(Boolean).filter(x=>normalizeText(x)!==normalizeText(q.word));
  if(family.length===0)return vocabQuestion({...q,_forceMode:'en-ja'});
  const targetWord=family[Math.floor(Math.random()*family.length)]; const target=familyMeta(targetWord,pool); const familyRecords=modeCandidateRecords(q,pool,'family',target.pos); const familyValues=familyRecords.flatMap(x=>String(x.family||'').split(';').map(s=>s.trim()).filter(Boolean)); const samePosFamily=familyValues.map(x=>({word:x,pos:familyMeta(x,pool).pos,level:(lookupVocab(x,pool)||{}).level||q.level})).filter(x=>normalizePos(x.pos)===normalizePos(target.pos));
  const answer=targetWord; const distractors=samePosFamily.concat(familyRecords.filter(x=>normalizeText(x.word)!==normalizeText(targetWord)));
  return {mode,question:`「${q.word}」の語族・派生語として適切なものを選べ。`,prompt:'同じ語族から適切な派生語を選べ。',answer,hidePos:true,choices:fillChoicesSmart(answer,distractors,q,target.pos||targetPos,levels,family)};
}
