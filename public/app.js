...
const APP_VERSION = '20260821-v16';
const scheduler = {
...
function renderLearningAnalytics() {
  ...
  const accuracy = vocabRecords.reduce((n,[,r])=>n+(r.answered||0),0) ? Math.round(vocabRecords.reduce((n,[,r])=>n+(r.correct||0),0)/vocabRecords.reduce((n,[,r])=>n+(r.answered||0),0)*100) : 0;
  ...
  panel.innerHTML=`<div class="analytics-head"><div><div class="analytics-title">学習状況の分析</div><div class="analytics-sub">実測ミスと、復習履歴・難度から算出した「要注意候補」を分けて表示します。</div></div><div class="analytics-sub">単語DB ${total.toLocaleString()}語</div></div><div class="analytics-grid"><div class="analytics-stat"><span>単語学習率</span><strong>${coverage}%</strong></div><div class="analytics-stat"><span>単語正答率</span><strong>${accuracy}%</strong></div><div class="analytics-stat"><span>学習済み単語</span><strong>${answeredUnique}</strong></div><div class="analytics-stat"><span>要注意候補</span><strong>${predicted.length}</strong></div></div><div class="analytics-columns"><div class="analytics-panel"><h4>間違えた単語</h4><ul class="analytics-list">${weakHtml}</ul></div><div class="analytics-panel"><h4>傾向的に間違えそうな単語</h4><ul class="analytics-list">${predHtml}</ul></div></div><div class="analytics-panel" style="margin-top:14px"><h4>難易度別の実績</h4>${levelHtml}</div>`;
  if (mini) mini.innerHTML=`<div class="analytics-title">学習分析</div><div style="font-size:11px;color:var(--muted);margin-top:5px">単語学習率 ${coverage}% / 正答率 ${accuracy}%</div>`;
}

async function loadAllVocabForAnalytics() {
  if (state.allVocab.length || !manifests?.vocab) return;
  const urls = urlsFor('vocab', Object.keys(manifests.vocab.levels || {}));
  const settled = await Promise.all(urls.map(async u=>{ try { const r=await fetch(`${u}?v=${APP_VERSION}`,{cache:'force-cache'}); const d=await r.json(); return Array.isArray(d)?d:[]; } catch { return []; }}));
  const seen=new Map();
  for(const q of settled.flat().map(normalizeQuestion).filter(Boolean)){ const k=`vocab:${q.id}`; if(!seen.has(k)) seen.set(k,q); }
  state.allVocab=[...seen.values()];
  renderLearningAnalytics();
}
...
function readingQuestion(q) {
  ...
  return {
    ...
    answer: q.answer || choices.find(c => c?.correct)?.text || '',
    choices,
...
function showReadingQuestion(q, opts = {}) {
  ...
  const box=$('answers');
  choices.forEach(text=>{ const b=document.createElement('button'); b.className='answer-btn'; b.textContent=typeof text==='string'?text:(text?.text||''); b.onclick=()=>answer(q,b.textContent===view.answer,b,view,opts); box.append(b); });
