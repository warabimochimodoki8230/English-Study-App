...
const APP_VERSION = '20260822-v23';
const scheduler = {
...
// Return a normalized progress record for a question.
// The answer handler relies on this function; keeping creation here prevents
// clicks from failing when a question has never been answered before.
function recordFor(q) {
  const k = key(q);
  let rec = state.score[k];
  ...
  // Make legacy/incomplete records safe for the scheduler and UI.
  ...
  rec.nextDue = Number(rec.nextDue || (rec.lastAnswered || Date.now()));
  rec.lastResult = rec.lastResult === 'correct' || rec.lastResult === 'wrong' ? rec.lastResult : null;
  rec.updatedAt = Number(rec.updatedAt || Date.now());
  return rec;
}
...
function renderLearningAnalytics() {
  ...
  const vocabByKey = new Map(state.allVocab.map(q => [key(q), q]));
  const wrongWords = vocabRecords
    ...
    .slice(0,8)
    .map(([k,r]) => ({ k, r, q: vocabByKey.get(k) }));
  ...
  const weakHtml = wrongWords.length ? wrongWords.map(({k,r,q})=>{ const label = q?.word || k.replace(/^vocab:/,'').replace(/^v-[^-]+-[^-]+-\d+-?/, ''); const meaning = q?.meaning ? ` <small style="color:var(--muted)">${escapeHtml(q.meaning)}</small>` : ''; return `<li><span>${escapeHtml(label)}${meaning}</span><span class="analytics-badge">${r.wrong||0}回ミス</span></li>`; }).join('') : '<li><span>まだ間違えた単語はありません。</span></li>';
...
function isVocab(q) { return Boolean(q && q.type === 'vocab'); }
...
async function loadAllVocabForAnalytics() {
  if (!manifests?.vocab?.levels) return;
  const urls = urlsFor('vocab', Object.keys(manifests.vocab.levels));
  ...
  state.allVocab = settled.flat().map(normalizeQuestion).filter(Boolean);
}
...
function normalizeQuestion(q) {
  ...
  // Preserve the source type first. Some idiom records also have a `word` field;
  // those must never be reclassified as vocabulary questions.
  if (q.type === 'vocab') {
    const word = String(q.word || q.answer || '').trim();
    const meaning = String(q.meaning || q.answerJa || q.ja || '').trim();
    const pos = normalizePos(q.pos || q.partOfSpeech || q.part_of_speech || inferredPos(word));
    return { ...q, type: 'vocab', word, meaning, pos };
  }
  if (q.type === 'idiom') {
...
function filterQuestions() {
  ...
  if (query) qs = qs.filter(x => `${x.word || ''} ${x.meaning || ''} ${x.ja || ''} ${x.question || ''} ${x.prompt || ''}`.toLowerCase().includes(query));
...
function choiceText(choice) {
  ...
  return String(choice.text ?? choice.label ?? choice.value ?? choice.word ?? choice.meaning ?? '').trim();
}
...
function normalizePos(pos) {
  ...
  const map = { '名':'n','名詞':'n','n':'n','noun':'n','動':'v','動詞':'v','v':'v','verb':'v','形':'adj','形容詞':'adj','adj':'adj','adjective':'adj','副':'adv','副詞':'adv','adv':'adv','adverb':'adv','前':'prep','前置詞':'prep','prep':'prep','preposition':'prep','接':'conj','接続詞':'conj','conj':'conj','conjunction':'conj','代':'pron','代名詞':'pron','pron':'pron','pronoun':'pron','限定詞':'det','det':'det','determiner':'det','間':'interj','間投詞':'interj','interj':'interj','interjection':'interj','冠':'article','冠詞':'article','article':'article','ord':'ord','序数':'ord' };
  return map[p] || p || 'n';
}
function posLabel(pos) { return `[${normalizePos(pos)}]`; }
function inferredPos(word) {
  const raw = String(word || '').toLowerCase();
  if (/(tion|sion|ment|ness|ity|ance|ence|ship|hood|dom|ism|er|or|ist)$/.test(raw)) return 'n';
  if (/ly$/.test(raw)) return 'adv';
  if (/(ous|ive|able|ible|ful|less|ic|ical|al)$/.test(raw)) return 'adj';
  if (/(ize|ise|ify|en)$/.test(raw)) return 'v';
  return 'n';
}
...
function familyMeta(word, pool) {
  const w = normalizeText(word);
  const hit = pool.find(x => isVocab(x) && normalizeText(x.word) === w);
  if (hit) return { word, pos: normalizePos(hit.pos), level: hit.level, record: hit };
  const raw = String(word || '').toLowerCase();
  let pos = '';
  if (/(tion|sion|ment|ness|ity|ance|ence|al)$/.test(raw)) pos = 'n';
  else if (/(ly)$/.test(raw)) pos = 'adv';
  else if (/(ous|ive|able|ible|ful|less|ic|al)$/.test(raw)) pos = 'adj';
  else if (/(ate|en|ify|ize|ise)$/.test(raw)) pos = 'v';
  return { word, pos };
}
function candidateRank(c, q, targetPos, preferredLevels = []) {
  const pos = normalizePos(c.pos);
  const samePos = targetPos && pos === targetPos ? 0 : 100;
  const lvl = rankLevelDistance(c.level, preferredLevels);
  const sameFamily = relatedValues(q,'family').some(f => normalizeText(f) === normalizeText(c.word)) ? 30 : 0;
  return samePos + lvl + sameFamily + Math.random() * 0.01;
}
function rankLevelDistance(level, preferredLevels) {
  if (!preferredLevels.length || !level) return 5;
  const r = levelRank[level] ?? 2;
  return Math.min(...preferredLevels.map(x => Math.abs(r - (levelRank[x] ?? 2)))) * 4;
}
function fillChoicesSmart(correct, candidates, q, targetPos = '', preferredLevels = [], exclude = []) {
  const answer = String(correct || '').trim();
  const seen = new Set([normalizeText(answer), ...exclude.map(normalizeText)]);
  const out = [];
  const uniq = candidates.filter(Boolean).filter(x => !seen.has(normalizeText(choiceText(x))))
    .sort((a,b) => candidateRank(a,q,targetPos,preferredLevels)-candidateRank(b,q,targetPos,preferredLevels));
  for (const c of uniq) {
    const text = choiceText(c); const n=normalizeText(text);
    if (!text || seen.has(n)) continue;
    seen.add(n); out.push(text);
...
function fillChoices(correct, candidates, minPool = [], answerRelated = []) {
  ...
  const out=[];
  const ranked=[...candidates,...minPool].filter(Boolean).sort(()=>Math.random()-0.5);
  for (const c of ranked) {
    const text=choiceText(c); const n=normalizeText(text);
    if (!text || seen.has(n)) continue;
    seen.add(n); out.push(text); if(out.length>=3) break;
  }
  return [answer,...out].slice(0,4);
}
function buildDistractors(q, pool, field) {
  const targetPos = normalizePos(q.pos);
  const sourceValues = field === 'meaning' ? x => x.meaning : field === 'word' ? x => x.word : x => x;
  const preferred = [q.level, levelOrderAdjacent(q.level)].filter(Boolean);
  const others = pool.filter(x => isVocab(x) && normalizeText(x.word) !== normalizeText(q.word));
  const samePos = targetPos ? others.filter(x => !x.pos || normalizePos(x.pos) === targetPos) : others;
  return fillChoicesSmart(sourceValues(q), samePos.map(sourceValues), q, targetPos, preferred);
}
function levelOrderAdjacent(level) {
  const r=levelRank[level] ?? 2;
  return Object.keys(levelRank).find(k=>levelRank[k] === Math.min(4,r+1)) || level;
}
function lookupVocab(word, pool = []) {
  const n = normalizeText(word);
  if (!n) return null;
  return (pool || []).find(x => isVocab(x) && normalizeText(x.word) === n)
    || (state.allVocab || []).find(x => isVocab(x) && normalizeText(x.word) === n)
    || null;
}
function familyMeta(word, pool) {
  const hit = lookupVocab(word, pool);
  return hit ? { word, pos: normalizePos(hit.pos), level: hit.level, record: hit } : { word, pos: inferredPos(word) };
}
function uniqueVocabRecords(pool = []) {
  const out = [], seen = new Set();
  for (const q of pool) {
    if (!isVocab(q)) continue;
    const k = normalizeText(q.word);
...
function candidateRank(c, q, targetPos, preferredLevels = []) {
  const pos = normalizePos(c.pos);
  const samePos = targetPos && pos === normalizePos(targetPos) ? 0 : 1000;
  const lvl = rankLevelDistance(c.level, preferredLevels);
  return samePos + lvl + Math.random() * 0.01;
}
function fillChoicesSmart(correct, candidates, q, targetPos = '', preferredLevels = [], exclude = []) {
  const answer = String(correct || '').trim();
  const seen = new Set([normalizeText(answer), ...exclude.map(normalizeText)]);
  const out = [];
  const uniq = (candidates || []).filter(Boolean)
    .filter(x => !seen.has(normalizeText(choiceText(x))))
    .sort((a,b) => candidateRank(a,q,targetPos,preferredLevels)-candidateRank(b,q,targetPos,preferredLevels));
  for (const c of uniq) {
    const text = choiceText(c); const n = normalizeText(text);
    if (!text || seen.has(n)) continue;
...
function modeCandidateRecords(q, pool, mode, targetPos = '') {
  const all = uniqueVocabRecords(pool).filter(x => normalizeText(x.word) !== normalizeText(q.word));
  const pos = normalizePos(targetPos || q.pos);
  const samePos = all.filter(x => normalizePos(x.pos) === pos);
  const preferredLevels = [q.level, levelOrderAdjacent(q.level)].filter(Boolean);
  const preferred = samePos.filter(x => preferredLevels.includes(x.level));
  return [...preferred, ...samePos.filter(x => !preferred.includes(x)), ...all.filter(x => !samePos.includes(x))];
}
function modeCandidateValues(q, pool, mode) {
  const recs = modeCandidateRecords(q, pool, mode, q.pos);
  if (mode === 'en-ja') return recs.map(x => x.meaning).filter(Boolean);
  if (mode === 'ja-en' || mode === 'context') return recs.map(x => x.word).filter(Boolean);
  if (mode === 'synonym') return recs.flatMap(x => Array.isArray(x.synonyms) ? x.synonyms : []).filter(Boolean);
  if (mode === 'family') return recs.flatMap(x => String(x.family || '').split(';').map(s => s.trim()).filter(Boolean));
  return [];
}
function modeCanGenerateFourChoices(q, mode, pool) {
  if (!isVocab(q)) return false;
  if (mode === 'context' && !q.exampleSentence) return false;
  if (mode === 'synonym' && (!Array.isArray(q.synonyms) || !q.synonyms.length)) return false;
  ...
  const related = mode === 'synonym' ? q.synonyms.slice(1) : mode === 'family' ? String(q.family || '').split(';').map(s => s.trim()).filter(Boolean).slice(1) : [];
  const candidates = modeCandidateRecords(q, pool, mode, mode === 'synonym' ? familyMeta(answer,pool).pos : mode === 'family' ? familyMeta(answer,pool).pos : q.pos);
  const values = mode === 'en-ja' ? candidates.map(x=>x.meaning)
    : mode === 'ja-en' || mode === 'context' ? candidates.map(x=>x.word)
    : mode === 'synonym' ? candidates.flatMap(x=>Array.isArray(x.synonyms)?x.synonyms:[]).concat(candidates.map(x=>x.word))
    : candidates.flatMap(x=>String(x.family||'').split(';').map(s=>s.trim()).filter(Boolean)).concat(candidates.map(x=>x.word));
  const seen = new Set([normalizeText(answer), ...related.map(normalizeText)]);
  let count = 0;
  for (const v of values) { const n = normalizeText(v); if (!n || seen.has(n)) continue; seen.add(n); if (++count >= 3) return true; }
  return false;
}
function pickVocabMode(q) {
  ...
  const eligible = ['en-ja','ja-en','context','synonym','family'];
  if (selected === 'random') {
    const ready = eligible.filter(m => modeCanGenerateFourChoices(q,m,pool));
    selected = (ready.length ? ready : ['en-ja'])[Math.floor(Math.random() * (ready.length ? ready.length : 1))];
  }
  if (!modeCanGenerateFourChoices(q, selected, pool)) selected = 'en-ja';
  return selected;
}
function vocabQuestion(q) {
  ...
  const targetPos = normalizePos(q.pos);
  const recs = modeCandidateRecords(q, pool, mode, targetPos);
  const levels = [q.level, levelOrderAdjacent(q.level)].filter(Boolean);
  if (mode === 'en-ja') return { mode, question:q.word, prompt:'最も適切な日本語の意味を選べ。', answer:q.meaning, pos:targetPos, choices:fillChoicesSmart(q.meaning,recs.map(x=>({...x,label:x.meaning})),q,targetPos,levels,[q.word]) };
  if (mode === 'ja-en') return { mode, question:q.meaning, prompt:'この日本語の意味に合う英単語を選べ。', answer:q.word, pos:targetPos, choices:fillChoicesSmart(q.word,recs.map(x=>({...x,label:x.word})),q,targetPos,levels,[q.meaning]) };
  if (mode === 'context') {
    const example = q.exampleSentence || '';
    const rx = new RegExp(q.word.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i');
    const sentence = rx.test(example) ? example.replace(rx,'_____') : example;
    return { mode, question:sentence, prompt:'文脈に最も適する語を選べ。', answer:q.word, pos:targetPos, choices:fillChoicesSmart(q.word,recs.map(x=>({...x,label:x.word})),q,targetPos,levels,[q.meaning]), extra:q.exampleJa||'' };
  }
  if (mode === 'synonym') {
    ...
    const synPos = familyMeta(answer,pool).pos || targetPos;
    const records = modeCandidateRecords(q,pool,mode,synPos);
    const distractors = records.flatMap(x=>Array.isArray(x.synonyms)?x.synonyms:[])
      .map(x=>({word:x,label:x,pos:synPos,level:q.level}))
      .concat(records.map(x=>({...x,label:(Array.isArray(x.synonyms)&&x.synonyms[0])||x.word})));
    return {mode,question:q.word,prompt:'最も近い意味の語を選べ。',answer,pos:targetPos,choices:fillChoicesSmart(answer,distractors,q,synPos,levels,synonyms.slice(1))};
  }
  const family = String(q.family||'').split(';').map(s=>s.trim()).filter(Boolean).filter(x=>normalizeText(x)!==normalizeText(q.word));
  if (!family.length) return vocabQuestion({...q,_forceMode:'en-ja'});
  const targetWord = family[Math.floor(Math.random()*family.length)];
  const target = familyMeta(targetWord,pool);
  const familyRecords = modeCandidateRecords(q,pool,'family',target.pos);
  const familyValues = familyRecords.flatMap(x=>String(x.family||'').split(';').map(s=>s.trim()).filter(Boolean));
  const samePosFamily = familyValues.map(x=>({word:x,pos:familyMeta(x,pool).pos,level:(lookupVocab(x,pool)||{}).level||q.level})).filter(x=>normalizePos(x.pos)===normalizePos(target.pos));
  const fallbackSamePos = uniqueVocabRecords(pool).filter(x=>normalizeText(x.word)!==normalizeText(q.word) && normalizePos(x.pos)===normalizePos(target.pos)).map(x=>({...x,label:x.word}));
  const distractors = samePosFamily.concat(familyRecords.filter(x=>normalizeText(x.word)!==normalizeText(targetWord))).concat(fallbackSamePos);
  return {mode,question:`「${q.word}」の語族・派生語として適切なものを選べ。`,prompt:'同じ語族から適切な派生語を選べ。',answer:targetWord,hidePos:true,choices:fillChoicesSmart(targetWord,distractors,q,target.pos||targetPos,levels,family)};
}
...
function genericQuestion(q) {
  ...
  return { mode: 'standard', question: q.question || q.word || q.phrase || '', prompt: q.prompt || '', answer, choices: fillChoices(answer, choices) };
}
...
function showQuestion(q = state.questions[state.index], opts = {}) {
  ...
  $('emptyState').style.display = 'none';
  area.innerHTML = `<div class="q-meta"><span class="tag">${escapeHtml(q.level || state.levels[0])}</span><span>${opts.customQuiz ? `${state.quiz.answered + 1} / ${state.quiz.remaining}` : opts.study ? '復習スケジュール' : `${state.index + 1} / ${state.questions.length}`}</span><span class="tag">${escapeHtml(vocabModes[view.mode]?.label || view.mode)}</span></div><div class="question">${escapeHtml(view.question)}${view.pos && !view.hidePos ? `<span class="vocab-pos">${escapeHtml(posLabel(view.pos))}</span>` : ''}<small>${escapeHtml(view.prompt || '')}</small></div><div class="answers" id="answers"></div><div id="explain"></div><div class="next-row"><button id="nextBtn" type="button" class="primary-btn next-btn" disabled>次の問題</button></div>`;
  const box = $('answers');
  const answerKey = normalizeText(canonicalAnswer(view.choices, view.answer));
  choices.forEach((text, index) => {
    const displayText = choiceText(text);
    const b = document.createElement('button');
    b.className = 'answer-btn';
    b.type = 'button';
    b.textContent = displayText;
    b.dataset.choiceIndex = String(index);
    b.dataset.answerKey = normalizeText(displayText);
    b.dataset.isCorrect = b.dataset.answerKey === answerKey ? 'true' : 'false';
    b.onclick = () => answer(q, b.dataset.isCorrect === 'true', b, view, opts);
    box.append(b);
  });
  $('nextBtn').onclick = () => {
...
function showReadingQuestion(q, opts = {}) {
  ...
  choices.forEach((text, index)=>{
    const displayText=choiceText(text);
    const b=document.createElement('button');
    b.type='button'; b.className='answer-btn'; b.textContent=displayText;
    b.dataset.choiceIndex=String(index);
    b.dataset.answerKey=normalizeText(displayText);
    b.dataset.isCorrect=b.dataset.answerKey===answerKey?'true':'false';
    b.onclick=()=>answer(q,b.dataset.isCorrect==='true',b,view,opts);
...
function answer(q, isCorrect, btn, view, opts = {}) {
  ...
  else { rec.wrong++; btn.classList.add('wrong'); document.querySelectorAll('.answer-btn').forEach(b => { if (b.dataset.isCorrect === 'true') b.classList.add('correct'); }); }
  if (state.quiz.active) state.quiz.answered++;
  schedule(rec, isCorrect);
  state.score[k] = rec;
  document.querySelectorAll('.answer-btn').forEach(b => { b.disabled = true; });
  let explanation = q.explanation || '';
  if ((q.type === 'vocab' || q.type === 'idiom') && q.exampleSentence) explanation += `${explanation ? '\n' : ''}例文：${q.exampleSentence}\n和訳：${q.exampleJa || '（和訳なし）'}`;
  if (!explanation && q.type !== 'vocab' && q.type !== 'idiom' && q.type !== 'reading') explanation = '問題文の文脈と選択肢の関係を確認しましょう。';
  ...
  if (nextBtn) { nextBtn.disabled = false; nextBtn.removeAttribute('aria-disabled'); nextBtn.classList.add('is-ready'); }
  saveProgress();
}
...
function resetAllProgress(){
  if (confirm('学習進捗をすべて削除します。よろしいですか？')) {
    ...
    state.index = 0;
    state.quiz.answered = 0;
    state.quiz.remaining = 0;
    state.quiz.active = false;
    resetQuestionArea({clearData:false});
    filterQuestions();
    renderAll();
  }
}
const resetProgressButton = $('resetProgress');
if (resetProgressButton) resetProgressButton.onclick = resetAllProgress;
const resetProgressMobileButton = $('resetProgressMobile');
if (resetProgressMobileButton) resetProgressMobileButton.onclick = resetAllProgress;

(async () => {
  loadProgress();
  renderAll();
  try { await bootManifests(); await fetchData(); await loadAllVocabForAnalytics(); renderLearningAnalytics(); }
